...
class Database:
    ...
    def get_user(self, telegram_id: int) -> Optional[Dict[str, Any]]:
        with self._get_conn() as conn:
            row = conn.execute("SELECT * FROM users WHERE telegram_id = ? AND is_active = 1", (telegram_id,)).fetchone()
            return dict(row) if row else None
    
    def get_user_by_crm_id(self, crm_id: int, role: str) -> Optional[Dict[str, Any]]:
        with self._get_conn() as conn:
            row = conn.execute("SELECT * FROM users WHERE crm_id = ? AND role = ? AND is_active = 1", (crm_id, role)).fetchone()
            return dict(row) if row else None
    ...
    def add_homework_file(self, lesson_id: int, file_id: str, file_name: str, file_type: str = "document"):
        with self._get_conn() as conn:
            conn.execute("INSERT INTO homework_files (lesson_id, file_id, file_name, file_type) VALUES (?,?,?,?)",
                        (lesson_id, file_id, file_name, file_type))
    
    def get_homework_files(self, lesson_id: int) -> List[Dict[str, Any]]:
        with self._get_conn() as conn:
            rows = conn.execute("SELECT * FROM homework_files WHERE lesson_id = ? ORDER BY uploaded_at DESC", (lesson_id,)).fetchall()
            return [dict(row) for row in rows]
    ...
    def get_transfer_request(self, request_id: int) -> Optional[Dict[str, Any]]:
        with self._get_conn() as conn:
            row = conn.execute("SELECT * FROM transfer_requests WHERE id = ?", (request_id,)).fetchone()
            return dict(row) if row else None
    ...
    def mark_escalation_sent(self, lesson_id: int, escalation_type: str = "unclosed_lesson"):
        with self._get_conn() as conn:
            conn.execute("INSERT INTO escalation_log (lesson_id, escalation_type) VALUES (?,?)", (lesson_id, escalation_type))
